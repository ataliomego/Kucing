...
bo='\e[1m'
red='\e[1;31mm'
rerple() { echo -e "='\e[1;33m'
tyblue='\e[1;36m'
purple() { echo -e "\\033[35;1m${*}\\033[0m"; }
tyblue() { ho -e "\\033[33;1m${*{*}\\033[0m"; }
...
apt-get --reinstall --fix-missing install -y libreadline-dev zlib1g-dev libssl-dev python2 screen curl jq bzip2 gzip coreutils rsyslog iftop htop zip unzip net-tools sed gnupg gnurmngr libxml-parseansport-https build-essential dirmngr libxml-parser-perl neofetch screenfetch git lsof openssl easy-rsa fail2ban tmux vnstat dropbear libsqlite3-dev socat cron bash-completion ntpdate xz-utils sudo apt-transport-https gnupg2 gnupg1 dnsutils lsb-release chrony
gem install lolcat
...
echo -e "dan Jika Ingin menggunakan{YELLOW}---${NC}"
echo ""

read -p "$( echo -e "${GREEN}Input Your Cecho ""

read -p "$( echo -e "${GREEN}Input Your Cecho ""

read -p "$( echo -e "${GREEN}Input Your Choose ? ${NC}(${YELLOW}1/2${Natic / Pring Auh Di Hapus
mkdi/usr/localr/local/bin/stunnel5
rm -fr /etc/nginxl
